...
export default function ChatContainer({ currentChat, socket }) {
  ...
  useEffect(async () => {
    ...
    const response = await axios.post(process.env.REACT_APP_BASE_URL +recieveMessageRoute, {
      from: data._id,
      to: currentChat._id,
    });
  ...
  const handleSendMsg = async (msg) => {
    ...
    await axios.post(process.env.REACT_APP_BASE_URL +sendMessageRoute, {
      from: data._id,
      to: currentChat._id,
      message: msg,
    });
...
const Container = styled.div`
  ...
  .chat-messages {
    ...
    &::-webkit-scrollbar {
      width: 0.2rem;
      &-thumb {
        background-color: #adff2f;  //scroll
        width: 0.1rem;
        border-radius: 1rem;
      }
    }
    ...
    .sended {
      ...
      .content {
        background-color: #6495ed; //chat sent
      }
    }
    .recieved {
